...
export function BookingModal({ onClose, ...props }: Props) {
  ...
  const onSubmit = useCallback(
    async (values: FormValues) => {
      try {
        ...
        const date = values.date.clone().set('hour', 0).set('minute', 0).set('second', 0);
        ...
        notification.success({
          message: `Reserve a slot on ${values.date.format(
            'DD MMM YYYY'
          )}, from ${values.from.format('HH:mm')} - to ${values.to.format('HH:mm')} successfully`,
          duration: 200,
        });
        onClose();
      } catch (e: any) {
  ...
  useEffect(() => {
    if (props.opened) form.setFieldsValue({ from: props.from, to: props.to, date: props.from });
      ...
      >
        <Inputs label="Date" style={{ gap: 8 }}>
          ...
          <Form.Item name="to" noStyle rules={[{ required: true }]}>
            <TimePicker showSecond={false} format="HH:mm" showNow={false} use12Hours={false} />
          </Form.Item>
        </Inputs>
      </Form>
    </Modal>
